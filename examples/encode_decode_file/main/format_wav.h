 ...
 *
 */
#define WAV_HEADER_PCM_DEFAULT(_sample_size, _bits_per_sample, _sample_rate, _num_of_channels) { \
    .descriptor_chunk = { \
        .chunk_id = {'R', 'I', 'F', 'F'}, \
        .chunk_size = (_sample_size) + sizeof(wav_header_t) - 8, \
    ...
    .fmt_chunk = { \
        ...
        .num_of_channels = (_num_of_channels), \
        .sample_rate = (_sample_rate), \
        .byte_rate = (_bits_per_sample) * (_sample_rate) * (_num_of_channels) / 8, \
        .block_align = (_bits_per_sample) * (_num_of_channels) / 8, \
        .bits_per_sample = (_bits_per_sample)\
    }, \
    .data_chunk = { \
        .subchunk_id = {'d', 'a', 't', 'a'}, \
        .subchunk_size = (_sample_size) \
    } \
}

wav_header_t wav_HEADER_PCM_DEFAULT(uint32_t _sample_size, uint16_t _bits_per_sample, uint32_t _sample_rate, uint16_t _num_of_channels) {
  return (wav_header_t) {
    .descriptor_chunk = {
        .chunk_id = {'R', 'I', 'F', 'F'},
        .chunk_size = (_sample_size) + sizeof(wav_header_t) - 8,
...
void print_wav_header(wav_header_t *wav_header)
{
    printf("WAV:print_wav_header:\n");
